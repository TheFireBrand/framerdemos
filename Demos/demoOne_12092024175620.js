 ...
 */export function CanvasScrollAnimation_1_2(props){const containerRef=useRef(null);const[loadingProgress,setLoadingProgress]=React.useState(0);const[loaded,setLoaded]=React.useState(false);useEffect(()=>{if(props.framePaths&&props.frameCount>0){const framePaths=Array.from({length:props.frameCount}).map((_,i)=>{const frameNumber=String(i+1).padStart(4,"0")// Pads the number to 4 digits
;return props.framePaths.replace(/(\d+)(?=.png$|jpg$)/,frameNumber);});// Log the frame paths to verify they are correct
console.log("Frame paths:",framePaths);let loadedImages=0;const images=[];framePaths.forEach(path=>{const img=new Image;img.src=path;img.onload=()=>{loadedImages+=1;setLoadingProgress(Math.floor(loadedImages/props.frameCount*100));images.push(img);if(loadedImages===props.frameCount){setLoaded(true);// Initialize CanvasScrollClip only after all images are loaded
if(containerRef.current){const canvasScrollClip=new CanvasScrollClip(containerRef.current,{framePath:props.framePaths,frameCount:props.frameCount,scrollArea:props.scrollArea});// Add custom event triggers if provided
...
if(props.onLoadComplete){props.onLoadComplete();}}}};img.onerror=()=>{console.error(`Failed to load image at path: ${path}`);};});}},[props.framePaths,props.frameCount,props.scrollArea]);return /*#__PURE__*/_jsxs(Frame,{size:"100%",background:props.background,children:[!loaded&&/*#__PURE__*/_jsxs(Frame,{size:"100%",style:{position:"fixed",top:0,left:0,display:"flex",justifyContent:"center",alignItems:"center",backgroundImage:`url(${props.loadingBackgroundImage})`,backgroundSize:"cover",backgroundColor:props.loadingBackgroundColor,color:props.loadingTextColor,fontSize:props.FontSize,fontFamily:props.loadingFontFamily,height:"100vh"},children:["Loading ",loadingProgress,"%"]}),/*#__PURE__*/_jsx(Frame,{ref:containerRef,size:"100%",background:loaded?"transparent":"rgba(0,0,0,0)"})]});}CanvasScrollAnimation_1_2.defaultProps={background:"#ffffff00",framePaths:"https://thefirebrand.github.io/TheGist/dna/dna200_0001.jpg",// "https://thefirebrand.github.io/TheGist/extcompimports/space/space-0001.png",
//   framePaths: "https://thefirebrand.github.io/TheGist/0001.png",
frameCount:47,scrollArea:3500,FontSize:24,loadingTextColor:"white",loadingBackgroundColor:"rgba(0,0,0,0.8)",loadingFontFamily:"Arial, sans-serif",loadingBackgroundImage:"",onLoadComplete:()=>console.log("Loading complete")};addPropertyControls(CanvasScrollAnimation_1_2,{background:{type:ControlType.Color,title:"Background"},framePaths:{type:ControlType.String,title:"Frame Path"},frameCount:{type:ControlType.Number,title:"Frame Count",min:1},scrollArea:{type:ControlType.Number,title:"Scroll Area",min:1e3},loadingFontFamily:{type:ControlType.Font,title:"Loading Font Family"},FontSize:{type:ControlType.Number,title:"Loading Font Size",min:"8",max:"100"},loadingTextColor:{type:ControlType.Color,title:"Loading Text Color"},loadingBackgroundImage:{type:ControlType.Image,title:"Loading Background Image"},onLoadComplete:{type:ControlType.EventHandler,title:"LoadComplete"}});export default CanvasScrollAnimation_1_2;CanvasScrollAnimation_1_2.displayName="Image Sequence Scroll Animation + Preloader";
